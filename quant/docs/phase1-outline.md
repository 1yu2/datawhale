...
> **待补充：** 运行 Notebook 的“2.1 股票数据介绍”后，在这里插入 AAPL 前 5 行数据截图，并写下你实际得到的行数和日期范围。
...
> **待补充：** 插入 Notebook 中“收盘价折线 + 成交量柱状图”，并用一句话描述价格的大致方向以及成交量是否出现过明显高峰。
...
> **待补充：** 插入你的“日收益率曲线 + Histogram”运行结果，并记录平均日收益率、日收益率标准差以及你观察到的最大尖峰位置。
...
> **待补充：** 插入累计收益率曲线，填写你运行时的区间累计收益率，并注明数据的起止日期。
...
> **待补充：** 插入三只股票的收益率对比图、波动柱状图和 Histogram，并填写你运行得到的三个标准差及其从大到小的排序。
...
1. **把价格高低当成收益好坏**
   一只股票从 20 元涨到 22 元，收益率是 10%；另一只从 200 元涨到 205 元，收益率只有 2.5%。比较表现时应看比例，而不是只看涨了几元。

2. **把 `NaN` 改成 0 后当作真实收益**
   第一行没有前一天，因此无法计算收益率。为了统计而删除它可以，但不能说第一天真实收益率就是 0。

3. **把平均日收益率直接乘以交易日数**
   收益会复利累积，准确的累计收益率应使用 `(1 + rets).cumprod() - 1`。

4. **认为标准差越大，收益一定越高**
   标准差只衡量波动幅度，不区分向上还是向下。大幅亏损同样会增大标准差。

5. **用不同时间范围直接比较**
   比较多只股票时，应尽量保持起止日期、数据频率和计算方式一致。

6. **把历史结果当成未来预测**
   本章结果只说明过去一段样本中发生了什么，历史表现不代表未来表现，也不构成投资建议。
